        ...
        <div class="main-grid">
            ...
            <div class="main-content">
                <div class="card">
                    ...
                    @if($errors->any())
                        <div class="error-box">
                            <span style="font-weight: 600;">Error!</span> {{ $errors->first() }}
                        </div>
                    @endif
                    ...
                    <form action="{{ route('mahasiswa.rps.store') }}" method="POST">
                        ...
                        <div class="table-wrapper">
                            <table>
                                ...
                                <tbody>
                                    @forelse($availableCourses as $mk)
                                        @php
                                        ...
                                        @endphp
                                        <tr class="{{ !$isAvailable && !$isWarning ? 'disabled-row' : '' }}">
                                            <td>
                                                <input type="checkbox" name="matakuliah_ids[]" value="{{ $mk->id }}" {{ !$isAvailable && !$isWarning ? 'disabled' : '' }}>
                                            </td>
                                            <td>{{ $mk->kode_mk }}</td>
                                            <td>
                                                {{ $mk->nama_mk }}
                                                @if ($prasyaratStatus)
                                                    <div class="prasyarat-info {{ $isWarning ? 'warning' : 'unmet' }}">
                                                        {{ $isWarning ? 'Peringatan' : 'Prasyarat belum terpenuhi' }}: {{ implode(', ', $prasyaratStatus['courses']) }}
                                                    </div>
                                                @endif
                                            </td>
                                            <td>{{ $mk->sks }}</td>
